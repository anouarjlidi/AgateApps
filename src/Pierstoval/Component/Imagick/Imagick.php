...
namespace Pierstoval\Component\Imagick;
...
class Imagick {
    ...
    public function get($var) {
        if ($var && isset($this->vars[$var])) {
            ...
            trigger_error('Imagick Error : var ' . $var . ' does not exist', E_USER_NOTICE);
            return null;
        }
    }

    function __construct($convert_path = ''){

        $this->convert_path = preg_replace('~[/\\\]$~isUu', '', $convert_path);
     ...
     */
    function init($source, $dest, $x_resize = 0, $y_resize = 0) {
        if (!file_exists($source)) {
            trigger_error('Imagick Error : source image does not exist', E_USER_ERROR);
            return false;
        }
        ...
        $this->cmd[0]       = '"' . $this->convert_path . '/convert" "' . $source . '" ';
    }
    ...
    public function setup($source, $dest, $x_resize = 0, $y_resize = 0, $convert_path = '') {
        if (!$convert_path) {
            $convert_path = $this->convert_path;
        }
    ...
    function text($size, $x, $y, $color, $font, $contents, $ratio = true) {
        ...
        if ($ratio === true) {
            $size = (int) ($size * 1.4);
        } //Le ratio de la taille du texte est différent entre GD et Imagick
        $x     = (int) $x;
        $y     = (int) $y;
        $color = (string) $color;
        $font  = (string) $font;
    ...
    public function ellipse($x_center, $y_center, $width, $height, $fill_color, $stroke_color = '', $angle_start = 0, $angle_end = 360) {
        ...
        $convert_cmd = '';
        if ($stroke_color) {
            $convert_cmd .= ' -stroke "' . $stroke_color . '"';
        }
