...
//        } // end foreach
            ...
            if (strpos($data['id'], $idToReplace) === 0) {
                ...
                if (!$exists) {
                    ...
                    if ($this->dryRun) {
                        ...
                        $this->output->writeln('Add new <info>reference</info> of type <comment>'.$entity.'</comment> with name <comment>'.$object->getName().'</comment>');
                    } else {
    ...
    {
        $datas = array_filter($this->datas, function ($element) {
            return isset($element['tags']) && $element['tags'] === 'routes';
        });
        ...
        foreach ($datas as $data) {
            ...
            if (strpos($data['id'], $idToReplace) === 0) {
                ...
                if (!$object) {
                    ...
                    if ($this->dryRun) {
                        $object->setId($data['id']);
                        $this->output->writeln('Add new <info>object</info> of type <comment>'.$entity.'</comment> with id <comment>'.$object->getId().'</comment>');
                    }
                }
            } else {
                $object = $repo->find($id);
                if (!$object) {
                    throw new \RuntimeException('Tried to find '.$tag.' object with id '.$id.'.');
                }
            }
