...
class ApiMapsController extends FOSRestController {
    ...
    public function initAction() {
        ...
        $map = $this->getDoctrine()->getManager()->getRepository('EsterenMapsBundle:Maps')->findOneBy(array('id'=>$id));

        if (!$map) {
            return $this->quit('Aucune carte trouvée', 404);
        }

        $route_tiles = urldecode($this->generateUrl('esterenmaps_maps_api_tile', array('zoom'=>'{zoom}','id'=>$map->getId(), 'x'=>'{x}','y'=>'{y}')));

        $img_size = $this->container->getParameter('esterenmaps.tile_size');
