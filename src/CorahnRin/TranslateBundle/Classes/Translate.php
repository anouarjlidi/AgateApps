<?php
namespace CorahnRin\CorahnRinPagesBundle\Translation;
...
class Translate {
	...
	function __construct() {
		...
		if (file_exists($this->file)) {
			$cnt = FileAndDir::get($this->file);
			$final = json_decode($cnt, true);
			if ($final) {
				$this->words = $final;
			}
		} else {
...
//			'user_id'=>Users::$id,
	 ...
	 */
	public function translate($txt, $return = false, $params = array()) {
		...
		if (count($params)) {
			...
			foreach ($params as $k => $v) {
				if (is_string($k)) {
					...
					$k = '{'.$k.'}';
					$params_string[$k] = $v;//Les chaînes de type {TEXT}
				} elseif (is_numeric($k)) {
					$params_numeric[] = $v;//Les accolades simples {}
				}
			}

			//Changement des chaînes
			foreach ($params_string as $k => $v) {
				$txt = str_replace($k, $v, $txt);//On remplace {TEXT} par son paramètre éponyme dans le tableau
			}
			foreach ($params_numeric as $v) {
				$txt = preg_replace('~\{\}~U', $v, $txt, 1);//On remplace {} par son équivalent dans le tableau numérique
			}
		}//Fin écriture paramètres

		if (defined('P_LANG') && P_LANG === 'fr') {//Par défaut si le site est en français, on n'a rien à traduire
