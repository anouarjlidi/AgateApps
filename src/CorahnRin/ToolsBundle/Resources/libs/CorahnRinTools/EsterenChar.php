...
class EsterenChar {
	 ...
	 */
	public function id($param = false) {
		if ($param === session_id()) {
			$this->id = $param;
		} return $this->id;
	}
	...
	public function name() {
		return $this->get('details_personnage.name');
	}
	...
	public function export_to_db($user_id = 0) {
		...
		if (!empty($search) && isset($search['nb_chars']) && $search['nb_chars'] > 0) {
			...
			if (!$user_id) {
				Session::setFlash('<br />Si vous souhaitez vraiment créer un personnage avec ce nom, vous pouvez créer un compte, ainsi le personnage sera enregistré pour vous avec ce nom.<br />Vous pouvez toujours changer le nom du personnage à l\'étape précédente', 'error');
			}
			return false;
		}
		$sql = '';
	...
	public function set($path = null, $value = null) {
		...
		return true; //On retourne le résultat de la fonction
	}
	...
	public function get($path = null) {
		if ($path === '' || $path === null) {
			...
			if (strpos($path, '.') === false) {
				if (array_key_exists($path, $this->char)) {
					$element = $this->char[$path];
				} else {
					$element = null;
				}
			} else {
	...
	private function _make_char_from_session($char) {
		...
		*/
		...
		&& isset($char['disciplines'])
			...
			if (isset($primsec['lettre'])) {
				$lettre = $primsec['lettre'];
			}
			...
			foreach($t as $v) {
				...
				$v['val'] = 0;
				if (isset($amelio[$id]))	{
					$v['val'] += (int) $amelio[$id]['primsec'] + (int) $amelio[$id]['curval'];
				}
				if (isset($bonusdom[$id]))	{
					$v['val'] += (int) $bonusdom[$id];
				}
				$domaines[$id] = array(
						'id' => (int) $v['domain_id'],
						'name' => $v['domain_name'],
		...
		&& isset($char['equipements']['autre_equip'])
			...
			if ($char['equipements']['arme']) {
				...
				foreach ($arme as $v) {
					...
					$armes_dom = array();
					foreach($doms as $d) {
						$armes_dom[$d] = $this->get('domaines.'.$d.'.name');
					}
		...
		if (isset($char['disciplines'])) {
			...
			if (!empty($disc)) {
				...
				$doms = array();
				foreach($disc as $k => $v) {
					$doms[$v['domain']] = $v['domain'];
				}//On récupère les domaines pour ne pas avoir de doublon de disciplines
				$discs = $this->db->req('
	...
	private function _make_pdf_from_original($printer_friendly = false) {
		...
		$av = array(); foreach($this->get('avantages') as $v) {
			$av[] = $v['name'].($v['val']>1 ? '    x'.$v['val'] : '');
		}
		if (isset($av[0])) {
		...
		$dv = array(); foreach($this->get('desavantages') as $v) {
			$dv[] = $v['name'].($v['val']>1 ? '    x'.$v['val'] : '');
		}
		if (isset($dv[0])) {
		...
		if ($this->get('inventaire.possessions')) {
			...
			foreach ($arr1 as $i => $v) {
				$pdf->textbox($v, 85, 535+$i*42.8, $p['times'], 14, 280);
			}
			foreach ($arr2 as $i => $v) {
				$pdf->textbox($v, 445, 535+$i*42.8, $p['times'], 14, 280);
			}
		}
		unset($i, $v, $arr1, $arr2);
	 ...
	 * @return boolean True si réussi, False sinon
	 */
	private function _make_sheet_from_original_gd($pages = false, $printer_friendly = false) {
		...
		$name = $this->id;
		$dest_folder = WEBROOT.DS.'files'.DS.'characters_export'.DS.$name;
		...
		/*--------------------------------*/
		if (in_array(1,$pages)) {
			$fiche = WEBROOT.DS.'files'.DS.'sheets'.DS.'esteren1'.($printer_friendly === true?'-pf':'').'.jpg';
			...
			// Avantages et désavantages
			$av = array(); foreach(($this->get('avantages') ? $this->get('avantages') : array()) as $v) {
				$av[] = $v['name'].($v['val']>1 ? '    x'.$v['val'] : '');
			}
			if (isset($av[0])) {
				imagettftext($nimg, 18, 0, 430, 500, $grey, $carolingia, substr($av[0], 0, 25));
			}
			if (isset($av[1])) {
				imagettftext($nimg, 18, 0, 430, 540, $grey, $carolingia, substr($av[1], 0, 25));
			}
			if (isset($av[2])) {
				imagettftext($nimg, 18, 0, 430, 580, $grey, $carolingia, substr($av[2], 0, 25));
			}
			if (isset($av[3])) {
				imagettftext($nimg, 18, 0, 430, 620, $grey, $carolingia, substr($av[3], 0, 25));
			}
			$dv = array(); foreach(($this->get('desavantages') ? $this->get('desavantages') : array()) as $v) {
				$dv[] = $v['name'].($v['val']>1 ? '    x'.$v['val'] : '');
			}
			if (isset($dv[0])) {
				imagettftext($nimg, 18, 0, 430, 685, $grey, $carolingia, substr($dv[0], 0, 25));
			}
			if (isset($dv[1])) {
				imagettftext($nimg, 18, 0, 430, 725, $grey, $carolingia, substr($dv[1], 0, 25));
			}
			if (isset($dv[2])) {
				imagettftext($nimg, 18, 0, 430, 765, $grey, $carolingia, substr($dv[2], 0, 25));
			}
			if (isset($dv[3])) {
				imagettftext($nimg, 18, 0, 430, 805, $grey, $carolingia, substr($dv[3], 0, 25));
			}
		...
		if (in_array(2,$pages)) {
			...
			$fiche = WEBROOT.DS.'files'.DS.'sheets'.DS.'esteren2'.($printer_friendly === true?'-pf':'').'.jpg';
			$img = imagecreatefromjpeg($fiche);
		...
		/*---------------------------------*/
		if (in_array(3,$pages)) {
			$fiche = WEBROOT.DS.'files'.DS.'sheets'.DS.'esteren3'.($printer_friendly === true?'-pf':'').'.jpg';
			$img = imagecreatefromjpeg($fiche);
	...
	private function _make_sheet_from_original($pages = false, $printer_friendly = false) {
		...
		if (in_array(1,$pages)) {
			...
			$imagick = new P_Imagick($fiche, $ret_names[0], null, null, 'C:/Program Files/ImageMagick-6.8.6-Q16/convert');
...
// 			imagecopyresampled($nimg, $img, 0, 0, 0, 0, $x, $y, $x, $y);
			...
			$imagick->text(25, 213, 280, $grey, $lettrine, $char_name);
			...
			$imagick->text(21, 880, 280, $grey, $lettrine, $player_name);
			unset($taille_du_texte, $police_du_texte, $char_name, $arr, $player_name);

			$imagick->text(18, 215, 322, $grey, $times, substr($this->get('details_personnage.sexe'), 0, 1));
			$imagick->text(20, 343, 322, $grey, $carolingia_bold, substr($this->get('age'), 0, 3));
			...
			foreach($desc as $i => $v) {
				if ($i == 0) {
					$imagick->text($taille_du_texte, 295, 365, $grey, $police_du_texte, $v);
				} elseif ($i == 1) {
					$imagick->text($taille_du_texte, 1115, 365, $grey, $police_du_texte, '(...)');
				} else {
					break;
				}
			}
			...
			$imagick->text(18, 530, 322, $grey, $lettrine, substr($this->get('peuple'), 0, 20));
			$imagick->text(18, 895, 322, $grey, $lettrine, substr($this->get('metier.name'), 0, 25));

			// voies
			$imagick->text(28, 325, 545, $grey, $carolingia_bold, $this->get('voies.1.val'));
			$imagick->text(28, 325, 608, $grey, $carolingia_bold, $this->get('voies.3.val'));
			$imagick->text(28, 325, 667, $grey, $carolingia_bold, $this->get('voies.2.val'));
			$imagick->text(28, 325, 727, $grey, $carolingia_bold, $this->get('voies.4.val'));
			$imagick->text(28, 325, 784, $grey, $carolingia_bold, $this->get('voies.5.val'));

			// voies des domaines ligne 1
			$imagick->text(22, 290, 990, $grey, $unzialish, $this->get('voies.2.val'));
			$imagick->text(22, 537, 990, $grey, $unzialish, $this->get('voies.4.val'));
			$imagick->text(22, 800, 990, $grey, $unzialish, $this->get('voies.4.val'));
			$imagick->text(22, 1069, 990, $grey, $unzialish, $this->get('voies.2.val'));
			// voies des domaines ligne 2
			$imagick->text(22, 298, 1169, $grey, $unzialish, $this->get('voies.1.val'));
			$imagick->text(22, 542, 1169, $grey, $unzialish, $this->get('voies.3.val'));
			$imagick->text(22, 802, 1169, $grey, $unzialish, $this->get('voies.5.val'));
			$imagick->text(22, 1060, 1169, $grey, $unzialish, $this->get('voies.4.val'));
			// voies des domaines ligne 3
			$imagick->text(22, 280, 1335, $grey, $unzialish, $this->get('voies.3.val'));
			$imagick->text(22, 540, 1335, $grey, $unzialish, $this->get('voies.3.val'));
			$imagick->text(22, 820, 1335, $grey, $unzialish, $this->get('voies.1.val'));
			$imagick->text(22, 1085, 1335, $grey, $unzialish, $this->get('voies.1.val'));
			// voies des domaines ligne 4
			$imagick->text(22, 271, 1502, $grey, $unzialish, $this->get('voies.4.val'));
			$imagick->text(22, 539, 1502, $grey, $unzialish, $this->get('voies.4.val'));
			$imagick->text(22, 800, 1502, $grey, $unzialish, $this->get('voies.3.val'));
			$imagick->text(22, 1065, 1502, $grey, $unzialish, $this->get('voies.3.val'));

			// Avantages et désavantages
			$av = array(); foreach(($this->get('avantages') ? $this->get('avantages') : array()) as $v) {
				$av[] = $v['name'].($v['val']>1 ? '    x'.$v['val'] : '');
			}
			if (isset($av[0])) { $imagick->text(18, 430, 500, $grey, $carolingia, substr($av[0], 0, 25)); }
			if (isset($av[1])) { $imagick->text(18, 430, 540, $grey, $carolingia, substr($av[1], 0, 25)); }
			if (isset($av[2])) { $imagick->text(18, 430, 580, $grey, $carolingia, substr($av[2], 0, 25)); }
			if (isset($av[3])) { $imagick->text(18, 430, 620, $grey, $carolingia, substr($av[3], 0, 25)); }
			$dv = array(); foreach(($this->get('desavantages') ? $this->get('desavantages') : array()) as $v) {
				$dv[] = $v['name'].($v['val']>1 ? '    x'.$v['val'] : '');
			}
			if (isset($dv[0])) { $imagick->text(18, 430, 685, $grey, $carolingia, substr($dv[0], 0, 25)); }
			if (isset($dv[1])) { $imagick->text(18, 430, 725, $grey, $carolingia, substr($dv[1], 0, 25)); }
			if (isset($dv[2])) { $imagick->text(18, 430, 765, $grey, $carolingia, substr($dv[2], 0, 25)); }
			if (isset($dv[3])) { $imagick->text(18, 430, 805, $grey, $carolingia, substr($dv[3], 0, 25)); }
			...
			$imagick->text(24, 920, 527, $brown, $times, $health['Bon']);
			$imagick->text(24, 920, 571, $brown, $times, $health['Moyen']);
			$imagick->text(24, 920, 615, $brown, $times, $health['Grave']);
			$imagick->text(24, 920, 658, $brown, $times, $health['Critique']);
			$imagick->text(24, 920, 700, $brown, $times, $health['Agonie']);

			$imagick->text(22, 1090, 755, $grey, $carolingia, $this->get('vigueur'));
			$imagick->text(22, 1090, 798, $grey, $carolingia, $this->get('survie'));

			// Domaines
			$x_arr = array(0,102,102,102,361,361,361,361,625,625,625,625,885,885,885,885,102) ;
			$y_arr = array(0, 977,1154,1320,977,1154,1322,1488,977,1154,1320,1488,977,1154,1320,1488,1488);
			$j = 0;
			foreach((array) $this->get('domaines') as $key => $val) {
				if (is_array($val) && isset($val['name']) && isset($val['val']) && isset($val['bonus']) && isset($val['malus'])) {
					$string = '';
					...
					if ($score >= 0) {
						for ($i = 1; $i <= $score; $i++) {
							$imagick->ellipse($x_arr[$j]+($i-1)*23.75-7, $y_arr[$j]+4, 8, 8, ($printer_friendly === true ? $darkgrey : $brown));
						}
					}
					if ($val['bonus']) {
						$imagick->text(16, $x_arr[$j]+41, $y_arr[$j]+34, $grey, $unzialish, '+'.$val['bonus']);
					}
					if ($val['malus']) {
						$imagick->text(16, $x_arr[$j]+132, $y_arr[$j]+34, $grey, $unzialish, '-'.$val['malus']);
					}
					$l = 0;
					foreach($val['disciplines'] as $v) {
						$imagick->text(13, $x_arr[$j]+36, $y_arr[$j]+57+$l*22, $darkgrey, $times, $v['name']);
						$imagick->text(17, $x_arr[$j]+211, $y_arr[$j]+57+$l*22, $darkgrey, $carolingia, $v['val']);
						$l++;
					}
				}
			}
...
// 			$img = imagecreatefromjpeg($fiche);
			...
			$imagick->setup($fiche, $ret_names[1]);
...
// 			imagecopyresampled($nimg, $img, 0, 0, 0, 0, $x, $y, $x, $y);
			...
			foreach((array) $this->get('inventaire.armes') as $v) {
				if (!is_array($v) && isset($v['name']) && isset($v['degats'])) {
					...
					$imagick->text(14, 123, 151+$i*43, $grey, $times, $v['name']);//Affichage de l'arme
					$imagick->text(20, 370, 157+$i*43-2, $grey, $carolingia, $v['degats']);
					$i++;
				}
			}

			$imagick->text(32, 335, 366, $grey, $carolingia, $this->get('potentiel'));
			...
			$imagick->text(13, 475, 115, $grey, $times, 'CàC/Tir');
			foreach($attitudes as $k => $v) {
				$imagick->text(15, 489, 161+$k*54, $grey, $carolingia_bold, $v['cac'].'/'.$v['tir']);
				$imagick->text(20, 572, 161+$k*54, $grey, $carolingia_bold, $v['def']);
				$imagick->text(20, 650, 161+$k*54, $grey, $carolingia_bold, $v['rap']);
			}
			...
			if ($this->get('defense.amelioration')) {
				for ($i = 1; $i <= $this->get('defense.amelioration'); $i++) {
					...
					$imagick->ellipse(756+($i-1)*27.8+$off, 124, 8, 8, ($printer_friendly === true ? $darkgrey : $brown));
				}
			}

			//Rapidité améliorée
			for ($i = 1; $i <= $this->get('rapidite.amelioration'); $i++) {
				$imagick->ellipse(756+($i-1)*27.8, 207, 8, 8, ($printer_friendly === true ? $darkgrey : $brown));
			}
			...
			if ($this->get('inventaire.armures')) {
				...
				foreach ($arr as $k => $v) {
					...
					$v = str_replace("\n", '', $v);
					$str = $v['name'].' ('.$v['protection'].')';
					$imagick->text(14, 750, 277+($i*31), $grey, $times, $str);
					$i++;
				}
			}
			...
			if ($this->get('arts_combat')) {
				...
				foreach ($this->get('arts_combat') as $v) {
					$str = $v['name'];
					$imagick->text(20, 448, 1026+($i*44), ($printer_friendly === true ? $darkgrey : $brown), $carolingia_bold, $str);
					$i++;
				}
			}
			...
			if ($this->get('inventaire.artefacts')) {
				...
				foreach($story as $i => $v) {
					if ($i <= 2) {
						$imagick->text($taille_du_texte, 92, 1028+$i*43, $grey, $police_du_texte, $v);
					} elseif ($i == 3) {
						$imagick->text($taille_du_texte, 390, 1115, $grey, $police_du_texte, '(...)');
					} else {
						break;
					}
				}
			...
			if ($this->get('inventaire.objets_precieux')) {
				...
				foreach($story as $i => $v) {
					if ($i <= 2) {
						$imagick->text($taille_du_texte, 800, 790+$i*43, $grey, $police_du_texte, $v);
					} elseif ($i == 3) {
						$imagick->text($taille_du_texte, 1105, 876, $grey, $police_du_texte, '(...)');
					} else {
						break;
					}
				}
			...
			if ($this->get('inventaire.possessions')) {
				...
				foreach($this->get('inventaire.possessions') as $k => $v) {
					...
					$imagick->text(14, $x_offset, 535+$i*42.8, $grey, $times, $equip_word);
					$i++;
				}
			}
			...
			if ($this->get('ogham')) {
				...
				foreach ($arr as $k => $v) {
					...
					$imagick->text(20, 147, 1377+$i*43, $grey, $carolingia_bold, $v);
					$i++;
				}
			}
			...
			if ($this->get('miracles')) {
				...
				foreach ($miracles as $k => $v) {
					...
					foreach($story as $i => $v) {
						if ($i <= 2) {
							$imagick->text($taille_du_texte, 457, 1341+$i*43+$offset, $grey, $police_du_texte, $v);
						} elseif ($i == 3) {
							$imagick->text($taille_du_texte, 750, 1426+$offset, $grey, $police_du_texte, '(...)');
						} else {
							break;
						}
					}
			...
			$imagick->text(32, 195, 1272, $grey, $carolingia_bold, $this->get('rindath.val').' / '.$this->get('rindath.max'));
			$imagick->text(32, 540, 1272, $grey, $carolingia_bold, $this->get('exaltation.val').' / '.$this->get('exaltation.val'));

			//Argent
			$argent = $this->get_daols($this->get('inventaire.argent'));
			$imagick->text(28, 830, 540, $grey, $carolingia_bold, $argent['braise']);
			$imagick->text(28, 830, 609, $grey, $carolingia_bold, $argent['azur']);
			$imagick->text(28, 830, 676, $grey, $carolingia_bold, $argent['givre']);
...
// 			}
		...
		if (in_array(3,$pages)) {
			...
			$imagick->setup($fiche, $ret_names[2]);
...
// 			imagecopyresampled($nimg, $img, 0, 0, 0, 0, $x, $y, $x, $y);
			...
			$resist_mentale = $this->get('resistance_mentale.val') + $this->get('resistance_mentale.exp');
			$imagick->text(25, 323, 528, $grey, $carolingia_bold, $resist_mentale);
			$imagick->text(28, 1050, 897,  $grey, $carolingia_bold, $this->get('voies.1.val'));
			$imagick->text(28, 1050, 969,  $grey, $carolingia_bold, $this->get('voies.2.val'));
			$imagick->text(28, 1050, 1041, $grey, $carolingia_bold, $this->get('voies.3.val'));
			$imagick->text(28, 1050, 1110, $grey, $carolingia_bold, $this->get('voies.4.val'));
			$imagick->text(28, 1050, 1180, $grey, $carolingia_bold, $this->get('voies.5.val'));
			...
			if ($this->get('details_personnage.histoire')) {
				...
				foreach($story as $i => $v) {
					if ($i <= 5) {
						$imagick->text($taille_du_texte, 90, 173+$i*43, $grey, $police_du_texte, $v);
					} elseif ($i == 6) {
						$imagick->text($taille_du_texte, 1100, 173+($i-1)*43, $grey, $police_du_texte, '(...)');
					} else {
						break;
					}
				}
			...
			$imagick->text(14, 557, 86, $grey, $carolingia, $this->get('region_naissance.royaume').' - '.$this->get('region_naissance.name').' - Milieu '.$this->get('residence_geographique'));

			$imagick->text(14, 557, 114, $grey, $carolingia, $this->get('classe_sociale'));
			...
			if ($this->get('revers')) {
				...
				$rev = implode(' - ', $rev);
				$imagick->text(14, 557, 142, $grey, $carolingia, $rev);
			}
			//Points de traumatisme
			$trauma = $this->get('traumatismes.permanents') + $this->get('traumatismes.curables');
			$off = 0;
			for ($i = 1; $i <= $trauma; $i++) {
				...
				$imagick->ellipse(231+($i-1)*27.8+$off, 582, 8, 8, $color);
			}
			...
			if ($this->get('endurcissement')) {
				...
				for ($i = 1; $i <= $endurcissement; $i++) {
					...
					$imagick->ellipse(231+($i-1)*27.8+$off, 618, 8, 8, $color);
				}
			}

			//Orientation
			$imagick->text(21, 271, 878, $grey, $carolingia_bold, $this->get('orientation.conscience'));
			$imagick->text(21, 420, 878, $grey, $carolingia_bold, $this->get('orientation.instinct'));
			$imagick->text(18, 645, 877, $grey, $carolingia_bold, $this->get('orientation.name'));

			//Désordre mental
			$imagick->text(21, 195, 674, $grey, $carolingia_bold, $this->get('desordre_mental.name'));

			//Qualité et défaut
			$imagick->text(21, 270, 940, $grey, $carolingia_bold, 'Qualité : '.$this->get('traits_caractere.qualite.name'));
			$imagick->text(21, 270, 982, $grey, $carolingia_bold, 'Défaut : '.$this->get('traits_caractere.defaut.name'));

			//Expérience
			$imagick->text(24, 679, 1325, $grey, $carolingia_bold,
			'Reste :  '.$this->get('experience.reste').'         Total :  '.$this->get('experience.total'));
			...
			if ($this->get('details_personnage.faits')) {
				...
				foreach($desc as $i => $v) {
					if ($i <= 5) {
						...
						$imagick->text($taille_du_texte, 176 + $offset, 1388+$i*43, $grey, $police_du_texte, $v);
					} elseif ($i == 6) {
						$imagick->text($taille_du_texte, 1102, 1388+($i-1)*43, $grey, $police_du_texte, '(...)');
					} else {
						break;
					}
				}
