...
class MakeFont {
	...
	public static function Message($txt, $severity='') {
		if(PHP_SAPI=='cli') {
			if($severity)
				echo "$severity: ";
			echo "$txt\n";
	...
	public static function Notice($txt) {
		self::Message($txt, 'Notice');
	}

	public static function Warning($txt) {
		self::Message($txt, 'Warning');
	}
	...
	public static function LoadMap($enc) {
		$file = ROOT.DS.'configs'.DS.'fpdf'.DS.'map'.DS.strtolower($enc).'.map';
		$a = file($file);
		if(empty($a))
			self::Error('Encoding not found: '.$enc);
	...
	public static function GetInfoFromTrueType($file, $embed, $map) {
		// Return informations from a TrueType font
		$ttf = new TTFParser();
		$ttf->Parse($file);
		if($embed) {
			if(!$ttf->Embeddable)
				self::Error('Font license does not allow embedding');
		...
		for($c=0;$c<=255;$c++) {
			if($map[$c]['name']!='.notdef') {
				...
				//else {
				//	self::Warning('Character '.$map[$c]['name'].' is missing');
				//}
			}
		}
		$info['Widths'] = $widths;
	...
	public static function GetInfoFromType1($file, $embed, $map) {
		// Return informations from a Type1 font
		if($embed) {
			$f = fopen($file, 'rb');
			if(!$f)
				self::Error('Can\'t open font file');
			// Read first segment
			$a = unpack('Cmarker/Ctype/Vsize', fread($f,6));
			if($a['marker']!=128)
				self::Error('Font file is not a valid binary Type1');
			$size1 = $a['size'];
			$data = fread($f, $size1);
			// Read second segment
			$a = unpack('Cmarker/Ctype/Vsize', fread($f,6));
		...
		if(!file_exists($afm))
			self::Error('AFM font file not found: '.$afm);
		$a = file($afm);
		if(empty($a))
			self::Error('AFM file empty or not readable');
		foreach($a as $line) {
			$e = explode(' ', rtrim($line));
			if(count($e)<2)
				continue;
		...
		if(!isset($info['FontName']))
			self::Error('FontName missing in AFM file');
		...
		for($c=0;$c<=255;$c++) {
			...
			if($name!='.notdef') {
				if(isset($cw[$name]))
					...
					self::Warning('Character '.$name.' is missing');
			}
		}
		$info['Widths'] = $widths;
		return $info;
	...
	public static function MakeFontDescriptor($info) {
		// Ascent
	...
	public static function MakeWidthArray($widths) {
		$s = "array(\n\t";
		for($c=0;$c<=255;$c++) {
			if(chr($c)=="'")
				$s .= "'\\''";
	...
	public static function MakeFontEncoding($map) {
		// Build differences from reference encoding
		$ref = self::LoadMap('cp1252');
		$s = '';
		$last = 0;
		for($c=32;$c<=255;$c++) {
	...
	public static function SaveToFile($file, $s, $mode) {
		$f = fopen($file, 'w'.$mode);
		if(!$f)
			self::Error('Can\'t write to file '.$file);
		fwrite($f, $s, strlen($s));
		fclose($f);
	}

	public static function MakeDefinitionFile($file, $type, $enc, $embed, $map, $info) {
		$s = "<?php\n";
		$s .= '$type = \''.$type."';\n";
		$s .= '$name = \''.$info['FontName']."';\n";
		$s .= '$desc = '.self::MakeFontDescriptor($info).";\n";
		$s .= '$up = '.$info['UnderlinePosition'].";\n";
		$s .= '$ut = '.$info['UnderlineThickness'].";\n";
		$s .= '$cw = '.self::MakeWidthArray($info['Widths']).";\n";
		$s .= '$enc = \''.$enc."';\n";
		$diff = self::MakeFontEncoding($map);
		if($diff)
			$s .= '$diff = \''.$diff."';\n";
		if($embed) {
			$s .= '$file = \''.$info['File']."';\n";
		...
		$s .= "?>\n";
		self::SaveToFile($file, $s, 't');
	}

	public static function MakeFont($fontfile, $enc='cp1252', $embed=true) {
		...
		if(!file_exists($fontfile))
			self::Error('Font file not found: '.$fontfile);
		...
		$type = 'Type1';
			...
			self::Error('Unrecognized font file extension: '.$ext);

		$map = self::LoadMap($enc);

		if($type=='TrueType')
			$info = self::GetInfoFromTrueType($fontfile, $embed, $map);
		else
			$info = self::GetInfoFromType1($fontfile, $embed, $map);

		$basename = substr(basename($fontfile), 0, -4);
		$basename = ROOT.DS.'configs'.DS.'fpdf'.DS.'fonts'.DS.$basename;
		if($embed) {
			if(function_exists('gzcompress')) {
				$file = $basename.'.z';
				self::SaveToFile($file, gzcompress($info['Data']), 'b');
				$info['File'] = $file;
				self::Message('Font file compressed: '.$file);
			} else {
				$info['File'] = basename($fontfile);
				self::Notice('Font file could not be compressed (zlib extension not available)');
			}
		}

		self::MakeDefinitionFile($basename.'.php', $type, $enc, $embed, $map, $info);
		self::Message('Font definition file generated: '.$basename.'.php');
	}
}
