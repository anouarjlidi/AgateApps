...
$setback_value = $this->getStepValue() ?: [];
...
if (!$nb_revers && !$setback_value && !$choice_available) {
    ...
    while ($loop_nb > 0) {
        ...
        if ($chosen->getId() == 1) {
            ...
            $setback_value[$chosen->getId()] = ['id' => $chosen->getId(), 'avoided' => false];// On l'ajoute aux revers

            // Et on relance le dé !
            $dice   = rand(0, (count($setbacks_dice_list) - 1));
            $chosen = $setbacks_dice_list[$dice];
            unset($setbacks_dice_list[$dice]);// On désactive le nouveau revers pioché
            $setback_value[$chosen->getId()] = ['id' => $chosen->getId(), 'avoided' => false];// Et on l'ajoute aux revers du personnage
        } elseif ($chosen->getId() == 10) {
            // Chance !
            unset($setbacks_dice_list[$dice]);// On désactive "Chance" pour ne pas l'obtenir à nouveau
            $setbacks_dice_list = array_values($setbacks_dice_list);

            $setback_value[$chosen->getId()] = ['id' => $chosen->getId(), 'avoided' => false];// On l'ajoute aux revers du personnage
            //
            ...
            $setback_value[$chosen->getId()] = ['id' => $chosen->getId(), 'avoided' => true];// Et on l'ajoute aux revers, en spécifiant bien qu'il a été évité
        } else {
